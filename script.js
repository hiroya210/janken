...
function renderComponent(myCard = true) {
  const wrapper = document.createElement('div');
  let cardName = winState.myCard;
  if (!myCard) {
    cardName = winState.opponentCard
  }
  let svgWrapper = '';
  ...
  wrapper.className = `choices ${cardName}-color chosen-card`;
  wrapper.innerHTML = `
    <div class="logo">
      ${svgWrapper}
    </div>
...
function renderPickedCard() {
  const pickedCard = document.getElementById("my-picked-card");
  const component = renderComponent();

  pickedCard.appendChild(component)
}

function renderOpponentCard() {
  const pickedCard = document.getElementById("opponent-card");
...
function removeOpponentCard() {
  const pickedCard = document.getElementById("opponent-card");
  const chosenCard = pickedCard.querySelector(".chosen-card");
  if (!chosenCard) {
    return;
  }
  pickedCard.removeChild(chosenCard)
}

function renderWinLoseText() {
  const elements = document.querySelectorAll(".win-state-wrapper");
  elements.forEach(winLose => {
    winLose.classList.add("active")
    let text = "YOU WIN"
    if (winState.isTie) {
      text = "IT'S A TIE"
    }
    else if (!winState.isWin) {
      text = "YOU LOSE"
    }
    winLose.querySelector(".win-lose-text").textContent = text;
  })
}

function hideWinLoseText() {
  const elements = document.querySelectorAll(".win-state-wrapper");
  elements.forEach(winLose => {
...
async function handleCardsClick(cardValue) {
  winState.myCard = cardValue;
  renderPickedCard();
  goToStepTwo();

  await initOpponentPick();
...
async function initOpponentPick() {
  await sleep(1000);
...
function initJankenLogic() {
  ...
  winState.isTie = myCard === opponentCard;
  switch (myCard) {
    case CARDS.SCISSORS:
      winState.isWin = Boolean(SCISSORS_WIN[opponentCard]);
      break;
    
    case CARDS.PAPER:
      winState.isWin = Boolean(PAPER_WIN[opponentCard]);
      break;

    case CARDS.ROCK:
      winState.isWin = Boolean(ROCK_WIN[opponentCard]);
      break;

    case CARDS.LIZARD:
      winState.isWin = Boolean(LIZARD_WIN[opponentCard]);
      break;

    case CARDS.SPOCK:
      winState.isWin = Boolean(SPOCK_WIN[opponentCard]);
      break;
  }
...
function resetState() {
  winState.myCard = "";
  winState.opponentCard = "";
  if (!winState.isTie && !winState.isWin) {
    winState.score = 0
  }
  winState.isWin = false;
  winState.isTie = false;
}
